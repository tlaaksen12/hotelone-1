...
1. 성능
   ...
   1. 예약 상태가 바뀔 때마다 카톡 등으로 알림을 줄 수 있어야 한다. Event driven
...
1. order의 주문, reservation의 예약과 취소, payment의 결제, customer의 카카오톡 알림 등은 그와 연결된 command 와 event 들에 의하여 트랜잭션이 유지되어야 하는 단위로 그들 끼리 묶어줌(바운디드 컨텍스트)
1. 도메인 서열 분리 
...
1. 예약 상태가 바뀔 때 마다 카카오톡 알림을 보낸다.(OK)
